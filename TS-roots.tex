...
We wish to build from $\mathcal{D}$ a mapping $y({\bf x})=f({\bf x}) + \varepsilon_\text{n}$, where $f({\bf x})$ is a regression function and $\varepsilon_\text{n} \sim \mathcal{N}(0,\sigma^2_\text{n})$  is an additive zero-mean Gaussian noise with variance $\sigma^2_\text{n}$.
This observation noise is assumed to be independent and identically distributed. We set $\sigma_\text{n} = 0$ for noiseless output observations.
...
\end{equation}
where $\kappa({\bf x},{\bf x}')$ is a positive semi-definite covariance function characterized by several hyperparameters.

By conditioning such prior knowledge on $\mathcal{D}$ and utilizing the observation noise assumption, we can show that the vector of observations $\{y({\bf x}^1),\dots,y({\bf x}^n)\}$ is distributed according to an $N$-variate Gaussian with zero mean and covariance matrix ${\bf C} = {\bf K} + \sigma^2_\text{n} {\bf I}_n$, where the $(i,j)$th element of ${\bf K}$ is $\kappa({\bf x}^i,{\bf x}^j)$ and ${\bf I}_n$ denotes an $n$-by-$n$ identity matrix.
...
We can sample $\mathcal{GP}(0, \kappa)$
efficiently, if we have a spectral representation of the covariance
function $\kappa(\mathbf{x}, \mathbf{x}')$ that converges quickly.
...
the case, we can represent the prior and posterior GP as $f(\mathbf{x}) = \sum_{i=0}^{\infty} w_i \sqrt{\lambda_i} \phi_i(\mathbf{x})$ and $f(\mathbf{x}) = \sum_{i=0}^{N-1} w_i \sqrt{\lambda_i} \phi_i(\mathbf{x})$, respectively.
We can sample the GP prior as a truncated Wiener series, see
